 ...
 */
  ...
  exit(0);
    ...
    printf("server: got connection from %s:%hu\n",inet_ntoa(their_addr.sin_addr), htons(their_addr.sin_port));

	struct sockaddr_in local;
	socklen_t length = sizeof(local);
	getsockname(new_fd,(struct sockaddr*)&local, &length);
	printf("server: local address %s:%hu\n", inet_ntoa(local.sin_addr), htons(local.sin_port));

	struct sockaddr_in peer;
	length = sizeof(peer);
	getpeername(new_fd,(struct sockaddr*)&peer, &length);
