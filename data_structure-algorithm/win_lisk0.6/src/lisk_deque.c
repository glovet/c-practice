...
n4 _deque_init(vpp deque_pptr, u4 unit_size, u4 map_size, u4 block_size)
{
	...
	if (IS_NL(ALLOC(deque_p, deque, sizeof(*deque))))	{
	...
	if (IS_NL(ALLOC(block_p, deque->map, size)))	{
		DEALLOC(deque);
	...
	if (IS_NL(ALLOC(vp, deque->map[deque->start.index].data, size)))	{
		DEALLOC(deque->map);
		DEALLOC(deque);
...
n4 deque_update_map(vp deque_ptr)
{
	...
	if (IS_LT(num << 1, deque->map_size))	{
		...
		if (IS_NL(ALLOC(block_p, map, size)))	{
