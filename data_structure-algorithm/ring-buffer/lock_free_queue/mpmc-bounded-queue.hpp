...
public:
        ...
        _tail_seq(0)
    {
        ...
        for (size_t i = 0; i < _size; ++i) {
            // _buffer[i].seq.store(i, boost::memory_order_relaxed);
        ...
        const T& data)
    {
        ...
        // size_t  head_seq = _head_seq.load(boost::memory_order_relaxed);
        ...
        for (;;) {
            ...
            if (dif == 0) {
                ...
                // if (_head_seq.compare_exchange_weak(head_seq, head_seq + 1, boost::memory_order_relaxed)) {
                ...
                // head_seq = _head_seq.load(boost::memory_order_relaxed);
    ...
    dequeue(
        T& data)
    {
        // size_t       tail_seq = _tail_seq.load(boost::memory_order_relaxed);
        ...
        for (;;) {
            ...
            if (dif == 0) {
                ...
                // weak compare is faster, but can return spurious results
                // which in this instance is OK, because it's in the loop
                // if (_tail_seq.compare_exchange_weak(tail_seq, tail_seq + 1, boost::memory_order_relaxed)) {
                if (_tail_seq.compare_exchange_weak(tail_seq, tail_seq + 1, std::memory_order_relaxed)) {
                    ...
                    // node->seq.store(tail_seq + _mask + 1, boost::memory_order_release);
                ...
                // under normal circumstances this branch should never be taken
                // tail_seq = _tail_seq.load(boost::memory_order_relaxed);
    ...
    int size()
    {
        // return _queue_size.load(boost::memory_order_relaxed);
...
private:
    ...
    {
        T                     data;
        // boost::atomic_size_t  seq;
        std::atomic_size_t  seq;
    };

    // typedef typename boost::aligned_storage<sizeof(node_t), boost::alignment_of<node_t>::value>::type aligned_node_t;
    ...
    // boost::atomic_size_t _head_seq;
    std::atomic_size_t _head_seq;
    // boost::atomic<int> _queue_size;
    std::atomic<int> _queue_size;
    cache_line_pad_t    _pad2;
    // boost::atomic_size_t _tail_seq;
    std::atomic_size_t _tail_seq;
