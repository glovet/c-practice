...
int main (int argc, char **argv) {
    ...
    if (2 < argc) {
        conf->server_port = strtoul(argv[2], (char**)NULL, 10);
    }
    if (3 < argc) {
    ...
    if (conf->password && strlen(conf->password)) {
        redisAsyncCommand(clt, auth_callback, NULL, "AUTH %s", conf->password);
    }
    redisAsyncCommand(clt, NULL, NULL, "SET key_test value_test");
